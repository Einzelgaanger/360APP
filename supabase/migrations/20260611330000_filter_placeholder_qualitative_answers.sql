-- Hide placeholder qualitative answers (Nil, N/A, etc.) from 360 dashboards and Growth Hub pulse.

CREATE OR REPLACE FUNCTION public.boom_meaningful_text_answer(t text)
RETURNS boolean
LANGUAGE sql IMMUTABLE AS $$
  SELECT COALESCE(btrim(t), '') <> ''
    AND length(btrim(t)) >= 2
    AND lower(regexp_replace(btrim(t), '[.!?,;:]+$', '')) NOT IN (
      'nil', 'nill', 'n/a', 'na', 'none', 'nothing', 'no comment', 'no comments',
      'not applicable', 'notapplicable', 'null', 'blank', 'empty',
      '-', '--', '—', '.', '..', '...', 'same', 'same as above', 'nope', 'no'
    );
$$;

COMMENT ON FUNCTION public.boom_meaningful_text_answer(text) IS
  'True when peer written feedback is substantive (excludes Nil, N/A, None, etc.).';

-- Patch get_eo_growth_hub_pulse: add meaningful filter on all text_answer pulls.
...
BEGIN
  ...
  IF peer_cnt >= 1 THEN
    ...
    ) ORDER BY section), '[]'::jsonb) INTO sections
    FROM (
      SELECT q.section, ROUND(AVG(a.score)::numeric, 2) AS avg_score, COUNT(a.score)::int AS response_count
