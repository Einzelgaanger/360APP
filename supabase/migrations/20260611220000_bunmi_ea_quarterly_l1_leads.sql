-- Bunmi (CEO) also completes EA quarterly manager reviews for Uche, Gisele, and Omotola.
-- GCEO assessor (epa_gceo) remains for executive self follow-up; this adds the standard ea_quarterly task.
-- Bunmi is excluded from monthly self-assessment.

CREATE OR REPLACE FUNCTION public.boom_monthly_self_allowed(_employee uuid)
RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT NOT COALESCE(
    lower(e.email) = lower('bunmi.akinyemiju@peopleos.co'),
    false
  )
...
COMMENT ON FUNCTION public.boom_monthly_self_allowed(uuid) IS
  'EO pilot: Bunmi (Group CEO) is excluded from monthly self-assessment.';
...
COMMENT ON FUNCTION public.boom_bunmi_ea_quarterly_l1(uuid) IS
  'True when reviewee is an L1 lead who receives an additional ea_quarterly task from Bunmi.';
  ...
  IF my_sub = eo THEN
    ...
    targets AS (
      ...
      WHERE p.reviewer_employee_id = me
        ...
        AND e.eo_appraisal_active
      UNION ALL
      -- Bunmi → Uche / Gisele / Omotola: EA quarterly in addition to GCEO assessor.
      SELECT 'ea_quarterly', e.id
      FROM public.eo_ea_quarterly_pairs p
      JOIN public.employees e ON e.id = p.reviewee_employee_id
      WHERE p.reviewer_employee_id = me
...
COMMENT ON FUNCTION public.get_review_assignments(text, text) IS
  'BOOM tasks. Bunmi gets ea_quarterly + GCEO assessor for Uche/Gisele/Omotola; GCEO only for Deyi.';

GRANT EXECUTE ON FUNCTION public.boom_bunmi_ea_quarterly_l1(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.boom_monthly_self_allowed(uuid) TO authenticated;
